...
abstract class referencelibrary_forms_abstract_AttachmentTarget extends Core_AbstractFormSmarty
{
	...
	public function postPersist()
	{
		...
		if ($coverUploads['name'] != '') {
			$this->attachmentModel->saveCoverAttachments($this->getDto()->getId(), $this->CoverAttachments->getValue(),
				$totalFileSize);
		}
		if ($this->attachmentModel != null) {
			$this->attachmentModel->saveSimpleAttachments($this->getDto()->getId(), $this->Attachments->getValue(),
				$totalFileSize);
		}
	...
	public function validateAttachments()
	{
		...
		if (isset($getID)) {
				...
				foreach ($newUploads["tmp_name"] as $k => $tmpname) {
					...
					if (strlen(trim($filename)) > 0) {
						...
						if (!in_array($extension, $allowedFiletypes)) {
							$this->AttachmentError->setValue('Uploaded files for Reference document must be in one the following formats: ' . implode(", ",
									$allowedFiletypes));
							$valid = false;
						}
					}
			...
			if ($coverUploads['name'][0] == '' and $toDelete[0] <> 0) {
				...
				$allowedCoverFiletypes = explode(",",
					Core_Config::get("referencelibrary", "attachable_cover_file_types"));
				...
				if (strlen(trim($filenameCover)) > 0) {
					...
					if (!in_array($extensionCover, $allowedCoverFiletypes)) {
						$this->AttachmentError->setValue('Uploaded files for Cover Pic Image must be in one the following formats : ' . implode(", ",
								$allowedCoverFiletypes));
						$valid = false;
					}
				}
			...
			foreach ($newUploads["tmp_name"] as $k => $tmpname) {
				...
				if (strlen(trim($filename)) > 0) {
					...
					if (!in_array($extension, $allowedFiletypes)) {
						...
						$imgurl = $lObjDocumentService->storeImageDocumentService('uploads', 'referencelibrary',
							$lStrFilename, $lBinData, 0, false);
		...
		if ($coverUploads['name'] != '') {
			...
			if (strlen(trim($filenameCover)) > 0) {
				...
				if (!in_array($extension, $allowedCoverFiletypes)) {
					...
					$imgurlCover = $lObjDocumentService->storeImageDocumentService('uploads', 'referencelibrary',
						$lStrFilename, $lBinDataCover, 0, false);

					if ($imgurlCover != null) {
						$urlsCover["new"][] = array(
							"name" => $filenameCover,
							"location" => $imgurlCover,
							"filesize" => strlen($lBinDataCover)
						);
					} else {
						throw new Core_Exception("Attachment storage error", "Attachment storage error");
						$this->AttachmentError->setValue('An error occured storing the file in the document repository. Please try again later.');
