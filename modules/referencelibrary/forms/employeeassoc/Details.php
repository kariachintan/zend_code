...
class referencelibrary_forms_employeeassoc_Details extends Core_Form
{
	...
	public function build()
	{
		...
		$element->setAttrib('onclick',
			'window.open(\'/survey/participants/csv\',\'csvwindow\',config=\'height=600,width=700,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,directories=no,status=no\');');
		$element->setAttrib('class', 'button');
		$element->setAttrib('style',
			'display: inline-block; margin-bottom: 10px; float: none; width: 120px; margin-left: 200px;');
		$this->addElement($element);
