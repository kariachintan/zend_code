...
abstract class referencelibrary_models_abstract_AttachmentTarget extends Core_Models_Abstract_Details {
        ...
        if (!empty($attachments)) {
            foreach ($attachments as $key => $att) {
                ...
                if ($fileUrl != null) {
                    ...
                    Amerch_Log::info("referencelibrary","AttachmentDTO","Store attachment " .print_r($attachDto, true));
                    ...
                    Amerch_Log::info("referencelibrary","AttachmentApplicabilityDTO","Store Attachment Applicability " .print_r($attAppDto, true));

                    EntityUtils::persist($attAppDto);
                } else {
                    throw new Core_Exception("An error occured storing attachment $filename in document repository.", "An error occured storing attachment {$att['destination']} in document repository.");
                }
            }
        }
    ...
    public function deleteAttachment($applicableId, $location = "", $id = '') {       
        ...
        if (!$location) {
            if ($id) {
                ...
                $attachmentApplicabilities = EntityUtils::getAll("phpnotes_services_quotation_dto_AttachmentApplicabilityDTO", $lObjCriteria);
    ...
    public function getAttachmentsInfo($id) {
        ...
        foreach ($attachmentDtos as $d) {
            ...
            $info[] = array('id' => $d->getId(), 'location' => urlencode($d->getLocation()), 'filename' => ellipsis($d->getName(), 45));
        }      
