...
class referencelibrary_models_library_Details extends referencelibrary_models_abstract_AttachmentTarget
{
	...
	public function assignEmployees($employeeNumbersStr, $reflibId, &$employeeNumbersArray, &$redundantEmployees)
	{
		...
		if ($deleteTotal > 0) {
			...
			Amerch_Log::info("referencelibrary", "assignemployees - Following Employees are removed ",
				print_r($removedEmployees, true));
		...
		if ($insertTotal > 0) {
			...
			Amerch_Log::debug("referencelibrary/employeeassoc/assignemployees", "Mapping already exists - Employees",
				print_r($redundantEmployees, true));
	...
	public function deleteEmployeeAttachmentMapping($refId, $employeeIds)
	{
		...
		foreach ($employeeIds as $employeeid) {
			...
			if (!$qresult) {
				...
				Amerch_Log::error('referencelibrary', 'employeeassoc',
					"Error deleting employees [" . implode(",", $employeeIds) . "] for REFLIBRARY HEADERID: $reflibid");
	...
	public function assignEmployeeToAttachment($reflibid, $employeeIds, &$redundantEmployees)
	{
		...
		if (!empty($employeeIds)) {
			...
			if (!$qresult) {
				...
				Amerch_Log::error('referencelibrary', 'employeeassoc',
					"Error adding employees [" . implode(",", $employeeIds) . "] to REFLIBRARY HEADERID: $reflibid");
				$result = "A system error occured while adding the employee to the attachment.";
	...
	public function deleteHeaderMappings($refId)
	{
		...
		if (!$qresult) {
			...
			Amerch_Log::error('referencelibrary', 'deleteAction',
				"Error deleting employees for REFLIBRARY HEADERID: $refId");
			$result = "A system error occured while adding the employee to the attachment.";

		} else {
			Amerch_Log::info('referencelibrary', 'deleteAction',
				"Suceessfully deleted all mappings for REFLIBRARY HEADERID: $refId");
		}
