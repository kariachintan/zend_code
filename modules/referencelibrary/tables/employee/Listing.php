...
class referencelibrary_tables_employee_Listing extends Core_Table {

    public function build() {
        $codes = new Core_Codes();

        $column = new Core_Table_Column('Employee Number');
        $column->setWidth(5);
        $column->addContent(new Core_Table_Column_Content_Text('EmployeeNum'));
        $column->setFilter(new Core_Table_Column_Filter_Text());
        $this->addColumn($column);

        $column = new Core_Table_Column('Name');
        $column->setWidth(15);
        $column->addContent(new Core_Table_Column_Content_Text('FullName'));
        $column->setFilter(new Core_Table_Column_Filter_Text());
        $this->addColumn($column);
        ...
        $column->setWidth(10);
        $options = $codes->getOptionsByCodeName(Core_Config::get('referencelibrary', 'codes_reference_docprioritytypes'));
