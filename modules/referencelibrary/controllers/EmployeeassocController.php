...
class referencelibrary_EmployeeassocController extends Core_Controller_Action {
    ...
    function assignemployeesAction() {
		...
		if ($this->_request->isPost()) {		
			...
			if ($lBlnValid == true) {
				...
				Amerch_Log::debug("referencelibrary_EmployeeassocController","postAction","Used Mem Before: " . memory_get_usage());
				$storedEmployeeNumbers = $refModel->assignEmployees($_POST['EmployeeNumbers'], $this->id,$formattedEmployeeArr,$redundantEmployees,false);
		...
		$this->view->controls->addItem(new Core_Control_Item_Custom("Assign / Remove",array('onclick'=>'initLoader();javascript: document.forms[\'fm\'].Core_Form_Submit.click();','href' => 'javascript:void(0)')));
    ...
    public function employeesassignedAction() {
		...
		Amerch_Log::info("referencelibrary_EmployeeassocController","postAction","Used Mem Feedback screen: " . memory_get_usage());
